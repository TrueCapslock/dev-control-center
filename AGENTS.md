...
- `@prokom-dev/config` — standalone, loads `prokom.config.js` (CJS)
- `@prokom-dev/status` — standalone, `StatusStore` with subscribe pattern
- `@prokom-dev/plugins` — standalone, plugin registry
- `@prokom-dev/core` — depends on config + status + plugins; owns `Runtime`, `EventBus`, `TaskRunner`
- `@prokom-dev/ui` — depends on core + config + status; Ink 7 + React 19, renders `App` component
- `@prokom-dev/cli` — depends on config + core + ui; bin entrypoint at `src/index.ts`
...
- Config is project-local `prokom.config.{mjs,cjs,js}` using `export default` (ESM) or `module.exports` (CJS); see `docs/config.md`.
...
- Current Phase 2: search/filter (`/` key), confirmation prompts for `confirm: true` commands, scrollable output (50 lines active / 10 inactive), git branch in header, status persistence in `.prokom/status.json`.
